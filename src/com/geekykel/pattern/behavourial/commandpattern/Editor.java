...
public class Editor {
    ...
    public void init() {
        JFrame frame = new JFrame("Text editor (type & use buttons, Kelvin!)");
        ...
        Editor editor = this;
        // Using Lambda
        ctrlC.addActionListener((ActionEvent e) -> executeCommand(new CopyCommand(editor)));
    ...
    private void executeCommand(CommandNew command) {
        if (command.execute()) {
    ...
    private void undo() {
        ...
        CommandNew command = history.pop();
        if (command != null) {
            command.undo();
        }
    }
